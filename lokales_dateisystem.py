...
# ============================================================
# Diese Tools sind nur verfuegbar, wenn die Abhaengigkeiten installiert sind.
# Sie ermoeglichen sichere Internet-Recherche ohne Dateisystem-Zugriff.
try:
    from internet_recherche import register_research_tools

    # Registriere alle Recherche-Tools beim Haupt-Server
    register_research_tools(mcp)
    print("[MCP] Internet-Recherche-Tools aktiviert.", file=sys.stderr)
except ImportError as e:
    print(f"[MCP] Internet-Recherche-Tools nicht verfuegbar: {e}", file=sys.stderr)
    print("[MCP] Installiere mit: pip install duckduckgo-search beautifulsoup4", file=sys.stderr)
...
def delete_file(path: str) -> dict:
    ...
    try:
        ...
        return {"error": f"Fehler beim Löschen: {e!s}"}
            ...
            for folder_name, folder_id in folders_map.items():
                try:
                    ...
                    result[folder_name] = str(
                        ...
                        .replace("AppData_Roaming", "AppData/Roaming")
                        .replace("AppData_Local", "AppData/Local")
                    )
        else:
            xdg_dirs = {
...
def empty_directory(path: str, keep_structure: bool = True) -> dict:
    ...
    try:
        ...
        if keep_structure:
            for root, _dirs, files in os.walk(abs_path, topdown=False):
                for f in files:
                    try:
...
def get_temp_directory() -> dict:
    ...
    try:
        ...
        def _get_size(item: dict) -> int:
            val = item.get("size")
            return val if isinstance(val, int) else 0

        temp_files.sort(key=_get_size, reverse=True)
